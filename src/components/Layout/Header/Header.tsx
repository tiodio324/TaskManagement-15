...
export const Header = observer(() => {
  const { isAuthenticated, currentRole, user, logout, openLoginModal } = authStore;
  const { pageTitle, toggleMobileMenu, mobileMenuOpen, navigate } = navigationStore;

  // Отображаемое имя: имя пользователя → роль → «Гость»
  ...
  // Подпись роли (показывается под именем, если имя есть)
        ...
        {isAuthenticated ? (
          <div className={styles.userInfo}>
            <div className={styles.userDetails}>
              ...
              {/* Показываем роль только если есть имя (чтобы не дублировать) */}
              {user.name && (
